...
static int def_log_part_br_abs_start_sector[] = {MIB_TO_SECTORS(20), MIB_TO_SECTORS(30), MIB_TO_SECTORS(40)};
...
void copy_mbr_n_br(u8 *disk)
{
    ...
    for (i = 0; i < ARRAY_SIZE(def_log_part_table); i++)
    {
        ...
        printk(KERN_INFO "sblkdev: br copied\n");
    }
}
...
{
    dev->capacity = MEMSIZE;
    dev->data = vmalloc(MEMSIZE*SECTOR_SIZE); //
    if (dev->data == NULL) {
